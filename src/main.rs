...
enum EventMsg { Chunk(u64, String), Done(u64), Error(u64, String), Models(u64, Vec<String>) }
...
struct App {
    ...
    api_input: String, name_input: String, greeting: String, scroll: u16, request_id: u64,
}
...
impl App {
    ...
    fn save(&self) -> Result<(), String> {
        ...
        let path = dir.join(FILE); fs::write(&path, serde_json::to_string_pretty(&self.config).map_err(|e| e.to_string())?).map_err(|e| e.to_string())?;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let (config, setup, load_error) = match load() {
    ...
    let mut app = App::new(config, setup, load_error);
...
fn loop_app<B: Backend>(term: &mut Terminal<B>, app: &mut App, tx: mpsc::Sender<EventMsg>, rx: mpsc::Receiver<EventMsg>) -> io::Result<()> {
    loop {
        while let Ok(e) = rx.try_recv() { match e {
            EventMsg::Chunk(id, s) if id == app.request_id => if let Some(m) = app.messages.last_mut() { m.content.push_str(&s); },
            EventMsg::Done(id) if id == app.request_id => { app.busy = false; app.status = "Ready".into(); },
            EventMsg::Error(id, e) if id == app.request_id => { app.busy = false; app.error = Some(e); app.status = "Error".into(); if app.messages.last().is_some_and(|m| m.content.is_empty()) { app.messages.pop(); } },
            EventMsg::Models(id, v) if id == app.request_id => { app.models = v; app.model_state.select(Some(0)); app.busy = false; app.status = "Ready".into(); choose_model(app); },
            _ => {}
        }}
...
fn key(app: &mut App, k: KeyEvent, tx: &mpsc::Sender<EventMsg>) -> bool {
    ...
    match k.code {
        KeyCode::Esc if app.busy => { app.request_id = app.request_id.wrapping_add(1); app.status = "Cancelled".into(); app.busy = false; },
...
fn send(app: &mut App, tx: &mpsc::Sender<EventMsg>) {
    let text = app.input.trim().to_string(); app.input.clear(); app.error=None; app.request_id = app.request_id.wrapping_add(1); let id = app.request_id;
    ...
    tokio::spawn(async move { if let Err(e)=request(&cfg,&hist,tx.clone(),id).await { let _=tx.send(EventMsg::Error(id,e)); } });
}
fn refresh_async(app:&mut App,tx:mpsc::Sender<EventMsg>){app.request_id=app.request_id.wrapping_add(1);let id=app.request_id;let cfg=app.config.clone();app.busy=true;std::thread::spawn(move||{let rt=tokio::runtime::Runtime::new().unwrap();let _=tx.send(match rt.block_on(list_models(&cfg)){Ok(v)=>EventMsg::Models(id,v),Err(e)=>EventMsg::Error(id,e)});});}
...
fn setup(f:&mut Frame,a:&App,r:Rect,step:u8){let title=match step{0=>"Select provider",1=>"API key",2=>"Your name",3=>"Invalid configuration",_=>"Setup"};let text=match step{0=>PROVIDERS.iter().map(|p|p.name).collect::<Vec<_>>().join("\n"),1=>"Paste your API key, then press Enter.".into(),2=>"Enter the name Term Code should call you.".into(),3=>a.error.clone().unwrap_or_else(||"Invalid Config.json".into()),_=>String::new()};f.render_widget(Paragraph::new(text).block(Block::default().borders(Borders::ALL).title(title)),r)}
