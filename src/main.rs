...
async fn google(c:&Config,cwd:&Path,hist:&[Msg],tx:mpsc::Sender<EventMsg>,id:u64)->Result<(),String>{let model=c.model.as_deref().ok_or("No model selected")?;let sys=tools::system_prompt(cwd);let contents=hist.iter().map(|m|json!({"role":if m.role=="assistant"{"model"}else{"user"},"parts":[{"text":m.content}]})).collect::<Vec<_>>();let h=http().await?;let r=h.post(format!("https://generativelanguage.googleapis.com/v1beta/models/{model}:streamGenerateContent")).query(&[("alt","sse")]).header("x-goog-api-key",&c.api_key).json(&json!({"contents":contents,"system_instruction":{"parts":[{"text":sys}]}})).send().await.map_err(|e|e.to_string())?;let s=r.status();if !s.is_success(){return Err(api_error(&r.json::<Value>().await.unwrap_or_default(),s))}sse(r.bytes_stream(),tx,"/candidates/0/content/parts/0/text",id).await}
async fn sse<S>(mut stream:S,tx:mpsc::Sender<EventMsg>,path:&str,id:u64)->Result<(),String>where S:Stream<Item=Result<bytes::Bytes,reqwest::Error>>+Unpin{let mut buf=String::new();let mut received=false;while let Some(x)=stream.next().await{let chunk=String::from_utf8_lossy(&x.map_err(|e|e.to_string())?).replace("\r\n","\n");buf.push_str(&chunk);while let Some(p)=buf.find("\n\n"){let block=buf[..p].to_owned();buf.drain(..p+2);for line in block.lines(){let data=line.strip_prefix("data:").map(str::trim).unwrap_or("");if data=="[DONE]"{let _=tx.send(EventMsg::Done(id));return Ok(())}if let Ok(v)=serde_json::from_str::<Value>(data){if let Some(s)=v.pointer(path).and_then(Value::as_str){received=true;let _=tx.send(EventMsg::Chunk(id,s.into()))}if v.pointer("/error/message").is_some(){return Err(api_error(&v,StatusCode::BAD_GATEWAY))}}}}}if !buf.trim().is_empty(){for line in buf.lines(){let data=line.strip_prefix("data:").map(str::trim).unwrap_or("");if let Ok(v)=serde_json::from_str::<Value>(data){if let Some(s)=v.pointer(path).and_then(Value::as_str){received=true;let _=tx.send(EventMsg::Chunk(id,s.into()))}if v.pointer("/error/message").is_some(){return Err(api_error(&v,StatusCode::BAD_GATEWAY))}}}}if !received{return Err("Model stream ended without a response".into())}let _=tx.send(EventMsg::Done(id));Ok(())}
