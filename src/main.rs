...
use std::{cell::Cell, fs, io, path::{Path, PathBuf}, sync::mpsc, time::{Duration, SystemTime, UNIX_EPOCH}};

const DIR: &str = ".nativestuff";
const FILE: &str = "Config.json";

#[derive(Clone, Serialize, Deserialize)]
pub struct Config { pub provider: String, pub api_key: String, #[serde(default)] pub model: Option<String>, #[serde(default)] pub user_name: Option<String> }
pub struct Provider { pub id: &'static str, pub name: &'static str }
pub const PROVIDERS: &[Provider] = &[Provider { id:"openai", name:"OpenAI" }, Provider { id:"anthropic", name:"Anthropic" }, Provider { id:"google", name:"Google" }, Provider { id:"deepseek", name:"DeepSeek" }, Provider { id:"qwen", name:"Alibaba/Qwen" }, Provider { id:"openrouter", name:"OpenRouter" }, Provider { id:"zhipu", name:"Zhipu AI" }];
#[derive(Clone)] pub struct Msg { pub role:String, pub content:String }
pub enum EventMsg { Chunk(u64,String), Done(u64), Error(u64,String), Models(u64,Vec<String>) }
#[derive(PartialEq,Eq)] pub enum Mode { Chat, Setup(u8), Models, Providers }

pub struct App {
    pub config: Config, pub messages: Vec<Msg>, pub input: String, pub input_cursor: usize, pub input_anchor: Option<usize>,
    pub status: String, pub error: Option<String>, pub busy: bool, pub mode: Mode, pub provider: usize, pub models: Vec<String>,
    pub model_state: ListState, pub provider_state: ListState, pub api_input: String, pub name_input: String, pub greeting: String,
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if app.error.is_none() && !setup { if let Err(e) = load_models(&mut app).await { app.error = Some(e); app.status = "Model discovery failed".into(); } }
...
fn key(a: &mut App, k: KeyEvent, tx: &mpsc::Sender<EventMsg>) -> bool {
    ...
    match k.code {
        ...
        KeyCode::Backspace if ctrl => { if !delete_selection(a) { let p = word_left(&a.input, a.input_cursor); let old = a.input_cursor; move_cursor(a, p, false); a.input_anchor = Some(p); a.input_cursor = old; let _ = delete_selection(a); } },
        KeyCode::Delete if ctrl => { if !delete_selection(a) { let p = word_right(&a.input, a.input_cursor); a.input_anchor = Some(a.input_cursor); a.input_cursor = p; let _ = delete_selection(a); } },
...
fn provider_key(a: &mut App, k: KeyEvent) { match k.code { KeyCode::Esc => a.mode = Mode::Chat, KeyCode::Up => a.provider_state.select(Some(a.provider_state.selected().unwrap_or(0).saturating_sub(1))), KeyCode::Down => { let i = a.provider_state.selected().unwrap_or(0); if i + 1 < PROVIDERS.len() { a.provider_state.select(Some(i + 1)) } }, KeyCode::Enter => if let Some(i) = a.provider_state.selected() { a.provider = i; a.api_input.clear(); a.mode = Mode::Setup(1); a.status = "Paste your API key".into() }, _ => {} } }
fn send(a: &mut App, tx: &mpsc::Sender<EventMsg>) { let text = a.input.trim().to_string(); a.input.clear(); a.input_cursor = 0; clear_selection(a); a.error = None; a.request_id = a.request_id.wrapping_add(1); let id = a.request_id; a.messages.push(Msg { role: "user".into(), content: text }); a.messages.push(Msg { role: "assistant".into(), content: String::new() }); a.busy = true; a.status = "Thinking...".into(); a.scroll = 0; let cfg = a.config.clone(); let cwd = a.cwd.clone(); let hist = a.messages.clone(); let tx = tx.clone(); let _ = tokio::spawn(async move { if let Err(e) = request(&cfg, &cwd, &hist, tx.clone(), id).await { let _ = tx.send(EventMsg::Error(id, e)); } }); }
fn send_followup(a: &mut App, tx: &mpsc::Sender<EventMsg>) { a.error = None; a.request_id = a.request_id.wrapping_add(1); let id = a.request_id; a.messages.push(Msg { role: "assistant".into(), content: String::new() }); a.busy = true; a.status = "Processing tool result...".into(); a.scroll = 0; let cfg = a.config.clone(); let cwd = a.cwd.clone(); let hist = a.messages.clone(); let tx = tx.clone(); let _ = tokio::spawn(async move { if let Err(e) = request(&cfg, &cwd, &hist, tx.clone(), id).await { let _ = tx.send(EventMsg::Error(id, e)); } }); }
pub async fn request_direct(c: &Config, hist: &[Msg], cwd: &Path) -> Result<String, String> { let (tx, rx) = mpsc::channel(); request(c, cwd, hist, tx, 99999).await?; let mut r = String::new(); while let Ok(m) = rx.try_recv() { if let EventMsg::Chunk(_, s) = m { r.push_str(&s) } } Ok(r) }
fn refresh_async(a: &mut App, tx: mpsc::Sender<EventMsg>) { a.request_id = a.request_id.wrapping_add(1); let id = a.request_id; let cfg = a.config.clone(); a.busy = true; let _ = std::thread::spawn(move || { let rt = match tokio::runtime::Runtime::new() { Ok(x) => x, Err(e) => { let _ = tx.send(EventMsg::Error(id, e.to_string())); return } }; let _ = tx.send(match rt.block_on(list_models(&cfg)) { Ok(v) => EventMsg::Models(id, v), Err(e) => EventMsg::Error(id, e) }); }); }
async fn load_models(a: &mut App) -> Result<(), String> { let v = list_models(&a.config).await?; a.models = v; a.model_state.select((!a.models.is_empty()).then_some(0)); choose_model(a); Ok(()) }
fn choose_model(a: &mut App) { if a.config.model.as_ref().is_some_and(|m| a.models.contains(m)) { return } if let Some(m) = a.models.first().cloned() { a.config.model = Some(m); let _ = a.save(); } }
...
async fn list_models(c: &Config) -> Result<Vec<String>, String> { let h = http().await?; let models = if let Some(b) = base(c) { let r = h.get(format!("{b}/models")).bearer_auth(&c.api_key).send().await.map_err(|e| e.to_string())?; let s = r.status(); let v: Value = r.json().await.map_err(|e| e.to_string())?; if !s.is_success() { return Err(api_error(&v, s)) } v["data"].as_array().map(|a| a.iter().filter_map(|x| x["id"].as_str().map(str::to_owned)).collect()).unwrap_or_default() } else { match c.provider.as_str() { "anthropic" => { let r = h.get("https://api.anthropic.com/v1/models").header("x-api-key", &c.api_key).header("anthropic-version", "2023-06-01").send().await.map_err(|e| e.to_string())?; let s = r.status(); let v: Value = r.json().await.map_err(|e| e.to_string())?; if !s.is_success() { return Err(api_error(&v, s)) } v["data"].as_array().map(|a| a.iter().filter_map(|x| x["id"].as_str().map(str::to_owned)).collect()).unwrap_or_default() }, "google" => { let mut models = Vec::new(); let mut page_token = None; loop { let mut req = h.get("https://generativelanguage.googleapis.com/v1beta/models").header("x-goog-api-key", &c.api_key).query(&[("pageSize", "1000")]); if let Some(t) = &page_token { req = req.query(&[("pageToken", t)]) } let r = req.send().await.map_err(|e| e.to_string())?; let status = r.status(); let v: Value = r.json().await.map_err(|e| e.to_string())?; if !status.is_success() { return Err(api_error(&v, status)) } if let Some(items) = v["models"].as_array() { models.extend(items.iter().filter(|x| x["supportedGenerationMethods"].as_array().is_some_and(|m| m.iter().any(|v| v.as_str() == Some("generateContent")))).filter_map(|x| x["name"].as_str().map(|n| n.trim_start_matches("models/").to_owned()))) } page_token = v["nextPageToken"].as_str().map(str::to_owned); if page_token.is_none() { break } } models }, _ => return Err("Unsupported provider".into()) } }; if models.is_empty() { return Err("Provider returned no usable models".into()) } Ok(models) }
async fn request(c: &Config, cwd: &Path, hist: &[Msg], tx: mpsc::Sender<EventMsg>, id: u64) -> Result<(), String> { match c.provider.as_str() { "anthropic" => anthropic(c, cwd, hist, tx, id).await, "google" => google(c, cwd, hist, tx, id).await, _ => openai(c, cwd, hist, tx, id).await } }
async fn openai(c: &Config, cwd: &Path, hist: &[Msg], tx: mpsc::Sender<EventMsg>, id: u64) -> Result<(), String> { let b = base(c).ok_or("Provider endpoint unavailable")?; let model = c.model.as_deref().ok_or("No model selected")?; let mut msgs = vec![json!({"role":"system","content":tools::system_prompt(cwd)})]; for m in hist { let role = if m.role == "system" { "user" } else { &m.role }; msgs.push(json!({"role":role,"content":m.content})) } let h = http().await?; let r = h.post(format!("{b}/chat/completions")).bearer_auth(&c.api_key).json(&json!({"model":model,"messages":msgs,"stream":true})).send().await.map_err(|e| e.to_string())?; let s = r.status(); if !s.is_success() { return Err(api_error(&r.json::<Value>().await.unwrap_or_default(), s)) } sse(r.bytes_stream(), tx, "/choices/0/delta/content", id).await }
async fn anthropic(c: &Config, cwd: &Path, hist: &[Msg], tx: mpsc::Sender<EventMsg>, id: u64) -> Result<(), String> { let model = c.model.as_deref().ok_or("No model selected")?; let sys = tools::system_prompt(cwd); let msgs = hist.iter().filter(|m| m.role != "system").map(|m| json!({"role":m.role,"content":m.content})).collect::<Vec<_>>(); let h = http().await?; let r = h.post("https://api.anthropic.com/v1/messages").header("x-api-key", &c.api_key).header("anthropic-version", "2023-06-01").json(&json!({"model":model,"max_tokens":4096,"system":sys,"messages":msgs,"stream":true})).send().await.map_err(|e| e.to_string())?; let s = r.status(); if !s.is_success() { return Err(api_error(&r.json::<Value>().await.unwrap_or_default(), s)) } sse(r.bytes_stream(), tx, "/delta/text", id).await }
async fn google(c: &Config, cwd: &Path, hist: &[Msg], tx: mpsc::Sender<EventMsg>, id: u64) -> Result<(), String> { let model = c.model.as_deref().ok_or("No model selected")?; let sys = tools::system_prompt(cwd); let contents = hist.iter().map(|m| json!({"role":if m.role == "assistant" { "model" } else { "user" },"parts":[{"text":m.content}]})).collect::<Vec<_>>(); let h = http().await?; let r = h.post(format!("https://generativelanguage.googleapis.com/v1beta/models/{model}:streamGenerateContent")).query(&[("alt", "sse")]).header("x-goog-api-key", &c.api_key).json(&json!({"contents":contents,"system_instruction":{"parts":[{"text":sys}]}})).send().await.map_err(|e| e.to_string())?; let s = r.status(); if !s.is_success() { return Err(api_error(&r.json::<Value>().await.unwrap_or_default(), s)) } sse(r.bytes_stream(), tx, "", id).await }
async fn sse<S>(mut stream: S, tx: mpsc::Sender<EventMsg>, path: &str, id: u64) -> Result<(), String> where S: Stream<Item = Result<bytes::Bytes, reqwest::Error>> + Unpin {
    let mut buf = String::new(); let mut received = false;
    while let Some(x) = stream.next().await {
        let chunk = String::from_utf8_lossy(&x.map_err(|e| e.to_string())?).replace("\r\n", "\n"); buf.push_str(&chunk);
        while let Some(p) = buf.find("\n\n") { let block = buf[..p].to_owned(); buf.drain(..p + 2); for line in block.lines() { if process_sse_line(line, &tx, path, id, &mut received)? { return Ok(()) } } }
    }
    if !buf.trim().is_empty() { for line in buf.lines() { if process_sse_line(line, &tx, path, id, &mut received)? { return Ok(()) } } }
    if !received { return Err("Model stream ended without a response".into()) }
    let _ = tx.send(EventMsg::Done(id)); Ok(())
}
fn process_sse_line(line: &str, tx: &mpsc::Sender<EventMsg>, path: &str, id: u64, received: &mut bool) -> Result<bool, String> {
    let data = line.strip_prefix("data:").map(str::trim).unwrap_or(""); if data.is_empty() { return Ok(false) } if data == "[DONE]" { let _ = tx.send(EventMsg::Done(id)); return Ok(true) }
    if let Ok(v) = serde_json::from_str::<Value>(data) {
        if v.pointer("/error/message").is_some() { return Err(api_error(&v, StatusCode::BAD_GATEWAY)) }
        if path.is_empty() {
            if let Some(parts) = v.pointer("/candidates/0/content/parts").and_then(Value::as_array) { for part in parts { if part.get("thought").and_then(Value::as_bool) == Some(true) { continue } if let Some(s) = part.get("text").and_then(Value::as_str) { *received = true; let _ = tx.send(EventMsg::Chunk(id, s.into())); } } }
        } else if let Some(s) = v.pointer(path).and_then(Value::as_str) { *received = true; let _ = tx.send(EventMsg::Chunk(id, s.into())); }
    }
    Ok(false)
}
...
#[cfg(test)] mod tests { use super::*; #[test] fn test_tab_completion() { let mut input = "/m".to_string(); tab_complete(&mut input); assert_eq!(input, "/model"); let mut input = "/p".to_string(); tab_complete(&mut input); assert_eq!(input, "/provider"); let mut input = "/c".to_string(); tab_complete(&mut input); assert_eq!(input, "/config"); tab_complete(&mut input); assert_eq!(input, "/clear"); let mut input = "/".to_string(); tab_complete(&mut input); assert_eq!(input, "/model") } }
