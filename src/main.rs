...
impl App {
    fn new(config: Config, setup: bool, error: Option<String>) -> Self {
        ...
        let mode = if setup { Mode::Setup(0) } else { Mode::Setup(3) };
        Self {
            config,
            messages: Vec::new(),
            input: String::new(),
            status: if setup { "Choose a provider" } else { "Fix Config.json and restart" }.into(),
            error,
            busy: false,
            mode,
            provider,
    ...
    fn save(&self) -> Result<(), String> {
        ...
        fs::write(&path, serde_json::to_string_pretty(&self.config).map_err(|e| e.to_string())?)
...
fn load() -> Result<Option<Config>, String> {
    ...
    if !PROVIDERS.iter().any(|p| p.id == c.provider) {
        return Err(format!("Wrong Config.json formatting: unsupported provider {}", c.provider));
    }
    Ok(Some(c))
}
fn greeting(name: Option<&str>) -> String {
    ...
    let v = if h < 12 {
        [format!("Good morning{n}."), format!("What should we work on{n}?")]
    } else if h < 18 {
        [format!("Good afternoon{n}."), format!("What should we do{n}?")]
    } else {
        [format!("Good evening{n}."), format!("Good to see you back{n}.")]
    };
        ...
        while let Ok(e) = rx.try_recv() {
            match e {
                ...
                EventMsg::Error(id, e) if id == app.request_id => {
                    ...
                    if app.messages.last().is_some_and(|m| m.content.is_empty()) { app.messages.pop(); }
                }
...
fn command(app: &mut App, tx: &mpsc::Sender<EventMsg>) -> bool {
    ...
    match c.as_str() {
        ...
        "/model" => {
            ...
            if app.models.is_empty() { refresh_async(app, tx.clone()); }
        }
        "/provider" => {
...
fn model_key(app: &mut App, k: KeyEvent) {
    match k.code {
        ...
        KeyCode::Down => {
            ...
            if i + 1 < app.models.len() { app.model_state.select(Some(i + 1)); }
        }
        KeyCode::Enter => if let Some(i) = app.model_state.selected() {
            if let Some(m) = app.models.get(i).cloned() {
                app.config.model = Some(m);
                if let Err(e) = app.save() { app.error = Some(e); } else { app.mode = Mode::Chat; }
            }
        },
        _ => {}
    }
...
fn provider_key(app: &mut App, k: KeyEvent) {
    match k.code {
        ...
        KeyCode::Down => {
            ...
            if i + 1 < PROVIDERS.len() { app.provider_state.select(Some(i + 1)); }
        }
        KeyCode::Enter => if let Some(i) = app.provider_state.selected() {
            app.provider = i;
            app.api_input.clear();
...
fn send(app: &mut App, tx: &mpsc::Sender<EventMsg>) {
    ...
    tokio::spawn(async move {
        if let Err(e) = request(&cfg, &hist, tx.clone(), id).await { let _ = tx.send(EventMsg::Error(id, e)); }
    });
}
...
async fn list_models(c: &Config) -> Result<Vec<String>, String> {
    ...
    let models = if let Some(b) = base(c) {
        ...
        match c.provider.as_str() {
            ...
            "google" => {
                let r = h.get("https://generativelanguage.googleapis.com/v1beta/models").header("x-goog-api-key", &c.api_key).send().await.map_err(|e| e.to_string())?;
                let s = r.status();
                let v: Value = r.json().await.map_err(|e| e.to_string())?;
                if !s.is_success() { return Err(api_error(&v, s)); }
                v["models"].as_array().map(|a| a.iter().filter(|x| x["supportedGenerationMethods"].as_array().is_some_and(|m| m.iter().any(|v| v.as_str() == Some("generateContent")))).filter_map(|x| x["name"].as_str().map(|n| n.trim_start_matches("models/").to_owned())).collect()).unwrap_or_default()
            }
...
async fn request(c: &Config, hist: &[Msg], tx: mpsc::Sender<EventMsg>, id: u64) -> Result<(), String> {
    match c.provider.as_str() {
...
async fn openai(c: &Config, hist: &[Msg], tx: mpsc::Sender<EventMsg>, id: u64) -> Result<(), String> {
    ...
    let msgs: Vec<Value> = hist.iter().map(|m| json!({"role":m.role,"content":m.content})).collect();
    let h = http().await?;
    let r = h.post(format!("{b}/chat/completions")).bearer_auth(&c.api_key).json(&json!({"model":model,"messages":msgs,"stream":true})).send().await.map_err(|e| e.to_string())?;
    let s = r.status();
    if !s.is_success() { return Err(api_error(&r.json::<Value>().await.unwrap_or_default(), s)); }
...
async fn anthropic(c: &Config, hist: &[Msg], tx: mpsc::Sender<EventMsg>, id: u64) -> Result<(), String> {
    let model = c.model.as_deref().ok_or("No model selected")?;
    let msgs: Vec<Value> = hist.iter().filter(|m| m.role != "system").map(|m| json!({"role":m.role,"content":m.content})).collect();
    let h = http().await?;
    let r = h.post("https://api.anthropic.com/v1/messages").header("x-api-key", &c.api_key).header("anthropic-version", "2023-06-01").json(&json!({"model":model,"max_tokens":4096,"messages":msgs,"stream":true})).send().await.map_err(|e| e.to_string())?;
    let s = r.status();
    if !s.is_success() { return Err(api_error(&r.json::<Value>().await.unwrap_or_default(), s)); }
...
async fn google(c: &Config, hist: &[Msg], tx: mpsc::Sender<EventMsg>, id: u64) -> Result<(), String> {
    ...
    let h = http().await?;
    let r = h.post(format!("https://generativelanguage.googleapis.com/v1beta/models/{model}:streamGenerateContent")).query(&[("alt", "sse")]).header("x-goog-api-key", &c.api_key).json(&json!({"contents":contents})).send().await.map_err(|e| e.to_string())?;
    let s = r.status();
    if !s.is_success() { return Err(api_error(&r.json::<Value>().await.unwrap_or_default(), s)); }
        ...
        while let Some(p) = buf.find("\n\n") {
            ...
            for line in block.lines() {
                ...
                if let Ok(v) = serde_json::from_str::<Value>(data) {
                    if let Some(s) = v.pointer(path).and_then(Value::as_str) { let _ = tx.send(EventMsg::Chunk(id, s.into())); }
                }
            }
        }
    }
...
fn draw(f: &mut Frame, a: &App) {
    let l = Layout::vertical([Constraint::Length(2), Constraint::Min(4), Constraint::Length(3), Constraint::Length(1)]).split(f.area());
    let head = if matches!(a.mode, Mode::Chat) { format!("Term Code  •  {}  •  {}", PROVIDERS[a.provider].name, a.config.model.as_deref().unwrap_or("auto")) } else { "Term Code".into() };
    ...
    f.render_widget(Paragraph::new(input).block(Block::default().borders(Borders::ALL)), l[2]);
    f.render_widget(Paragraph::new(a.error.as_deref().unwrap_or(&a.status)), l[3]);
}
fn chat(f: &mut Frame, a: &App, r: Rect) {
    let mut s = a.messages.iter().map(|m| format!("{}:\n{}\n", if m.role == "user" { "You" } else { "Term Code" }, m.content)).collect::<Vec<_>>().join("\n");
    if s.is_empty() { s = a.greeting.clone(); }
    f.render_widget(Paragraph::new(s).scroll((a.scroll, 0)).wrap(Wrap { trim: false }).block(Block::default().borders(Borders::ALL)), r);
}
fn setup(f: &mut Frame, a: &App, r: Rect, step: u8) {
    let title = match step { 0 => "Select provider", 1 => "API key", 2 => "Your name", 3 => "Invalid configuration", _ => "Setup" };
    let text = match step {
        0 => PROVIDERS.iter().enumerate().map(|(i,p)| format!("{} {}", if i == a.provider { ">" } else { " " }, p.name)).collect::<Vec<_>>().join("\n"),
    ...
    f.render_widget(Paragraph::new(text).block(Block::default().borders(Borders::ALL).title(title)), r);
}
fn list(f: &mut Frame, v: &[String], st: &ListState, r: Rect, title: &str) {
    let mut s = st.clone();
    f.render_stateful_widget(List::new(v.iter().map(|x| ListItem::new(x.clone())).collect::<Vec<_>>()).highlight_symbol("> ").block(Block::default().borders(Borders::ALL).title(title)), r, &mut s);
}
