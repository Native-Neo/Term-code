...
fn load() -> Result<Option<Config>, String> {
    ...
    let c: Config =
        serde_json::from_str(&s).map_err(|e| format!("Wrong Config.json formatting: {e}"))?;
...
fn text(s: &mut String, k: KeyEvent, enter: impl FnOnce()) {
    match k.code {
        KeyCode::Enter => enter(),
        KeyCode::Backspace => {
            s.pop();
...
fn choose_model(app: &mut App) {
    if let Some(c) = app.config.model.as_ref() {
        if app.models.contains(c) {
            return;
        }
    }
...
async fn google(
    ...
    let r=h.post(format!("https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent?key={}",c.api_key)).json(&json!({"contents":contents})).send().await.map_err(|e|e.to_string())?;
    let s = r.status();
    let v: Value = r.json().await.map_err(|e| e.to_string())?;
    if !s.is_success() {
        return Err(api_error(&v, s));
    ...
    {
        let _ = tx.send(EventMsg::Chunk(id, x.into()));
    }
    ...
    S: Stream<Item = Result<bytes::Bytes, reqwest::Error>> + Unpin,
{
    ...
    while let Some(x) = stream.next().await {
        buf.push_str(&String::from_utf8_lossy(&x.map_err(|e| e.to_string())?));
        while let Some(p) = buf.find("\n\n") {
            let block = buf[..p].to_owned();
            buf.drain(..p + 2);
            for line in block.lines() {
                let d = line.strip_prefix("data: ").unwrap_or("");
                if d == "[DONE]" {
                    let _ = tx.send(EventMsg::Done(id));
                    return Ok(());
                }
...
fn draw(f: &mut Frame, a: &App) {
    ...
    f.render_widget(
        Paragraph::new(head).alignment(Alignment::Center).bold(),
        l[0],
    );
    match a.mode {
